...
class TestMain:
    ...
    def test_main_exception_handling(self, mock_sys, mock_logging, mock_print, 
                                   mock_gemini, mock_file_repo, mock_ragas, mock_use_case):
        """메인 함수 예외 처리 테스트"""
        # 예외 발생 설정
    ...
    @patch('src.presentation.main.FileRepositoryAdapter')
    @patch('src.presentation.main.GeminiAdapter')
    def test_dependency_injection_setup(self, mock_gemini, mock_file_repo, mock_ragas, mock_use_case):
        """의존성 주입 설정 테스트"""
        # Mock 설정
        mock_llm_instance = MagicMock()
        mock_gemini.return_value = mock_llm_instance
...
class TestMainConfiguration:
    ...
    def test_gemini_adapter_configuration(self, mock_gemini):
        ...
        # 다른 의존성들도 mock
        with patch('src.presentation.main.FileRepositoryAdapter'), \
             patch('src.presentation.main.RagasEvalAdapter'), \
    ...
    def test_file_repository_configuration(self, mock_file_repo):
        ...
        # 다른 의존성들도 mock
        with patch('src.presentation.main.GeminiAdapter'), \
             patch('src.presentation.main.RagasEvalAdapter'), \
             patch('src.presentation.main.RunEvaluationUseCase') as mock_use_case:
