...
class TestSQLiteAdapterInitialization:
    ...
    def test_init_with_default_path(self):
        """기본 경로로 초기화 테스트"""
        with patch('src.infrastructure.repository.sqlite_adapter.DATABASE_PATH') as mock_path:
    ...
    def test_database_schema(self, sqlite_adapter, temp_db_path):
        ...
        expected_columns = {
            'id', 'timestamp', 'faithfulness', 'answer_relevancy',
...
class TestSQLiteAdapterStatistics:
    ...
    def test_get_statistics_with_data(self, sqlite_adapter):
        ...
        assert abs(stats['average_scores']['faithfulness'] - 0.85) < 0.01  # (0.8 + 0.9) / 2
        assert abs(stats['average_scores']['answer_relevancy'] - 0.85) < 0.01  # (0.9 + 0.8) / 2
