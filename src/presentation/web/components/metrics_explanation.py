...
def show_metrics_overview_chart():
    ...
    with col2:
        ...
        **⚡ 실제 RAG 시스템 개발 경험:**
        ...
        - Answer Correctness는 Ground Truth 품질에 크게 의존적
        """
        )
...
def show_answer_correctness_explanation():
    ...
    with col1:
        ...
        **"답변이 정답과 얼마나 일치하나요?"**를 측정하는 지표입니다.
        ...
        AI가 생성한 답변과 사람이 작성한 정답(Ground Truth) 사이의 
        정확도를 측정합니다. 단순히 문자열 비교가 아니라 의미적 유사성과 
        사실적 정확성을 종합적으로 평가합니다.
        
        ### 📝 **구체적인 예시**
        
        **질문**: "파이썬에서 리스트의 길이를 구하는 방법은?"
        
        **정답 (Ground Truth)**: "len() 함수를 사용하여 리스트의 길이를 구할 수 있습니다."
        
        ✅ **좋은 답변 (Correctness = 0.95)**:
        "파이썬에서는 len() 함수를 사용해서 리스트의 길이를 구할 수 있습니다."
        → 의미와 정보가 정답과 거의 일치합니다!
        
        ⚠️ **보통 답변 (Correctness = 0.7)**:
        "리스트의 길이는 len() 함수로 구할 수 있고, 또는 반복문으로도 계산할 수 있습니다."
        → 핵심은 맞지만 추가 정보가 있어 완전히 일치하지 않습니다.
        
        ❌ **나쁜 답변 (Correctness = 0.2)**:
        "리스트의 크기는 size() 메서드를 사용하면 됩니다."
        → 잘못된 정보입니다! (Python에는 size() 메서드가 없음)
        """
        )
    ...
    with col2:
        ...
        fig = go.Figure(
            go.Indicator(
                mode="gauge+number",
                value=0.75,
                title={"text": "Answer Correctness 점수"},
                domain={"x": [0, 1], "y": [0, 1]},
                gauge={
                    "axis": {"range": [None, 1]},
                    "bar": {"color": "orange"},
    ...
    Answer Correctness는 두 가지 핵심 요소로 구성됩니다:
    
    1. **사실적 정확성 (Factual Accuracy)** (60%)
       - 답변에 포함된 사실들이 정답과 일치하는가?
       - 틀린 정보나 환각(hallucination)이 없는가?
    
    2. **의미적 유사성 (Semantic Similarity)** (40%)
       - 답변의 전체적인 의미가 정답과 유사한가?
       - 다른 표현이지만 같은 의미를 전달하는가?
    ...
    - **낮은 점수의 원인**: 모델 성능 부족, 부정확한 컨텍스트, 또는 Ground Truth 품질 문제
    - **다른 메트릭과의 관계**: Faithfulness가 높아도 Answer Correctness가 낮을 수 있음 (정답과 다른 정확한 정보 제공)
    - **개선 우선순위**: 다른 메트릭들이 높은 수준이라면, 모델 자체의 성능 개선에 집중
    """
    )
